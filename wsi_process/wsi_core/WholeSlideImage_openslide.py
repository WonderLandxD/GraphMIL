...
class WholeSlideImage_os(object):
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def isInContours(cont_check_fn, pt, holes=None, patch_size=256):
        if cont_check_fn(pt):
            if holes is not None:
                return not WholeSlideImage_os.isInHoles(holes, pt, patch_size)
            else:
                return 1
        return 0
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        results = pool.starmap(WholeSlideImage_os.process_coord_candidate, iterable)
        pool.close()
    ...
    def process_coord_candidate(coord, contour_holes, ref_patch_size, cont_check_fn):
        if WholeSlideImage_os.isInContours(cont_check_fn, coord, contour_holes, ref_patch_size):
            return coord
        else:
            return None
