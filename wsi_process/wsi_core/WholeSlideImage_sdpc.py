...
class WholeSlideImage_sdpc(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up=255, mthresh=7, close=0, use_otsu=False,
                      filter_params={'a_t': 100}, ref_patch_size=512, exclude_ids=[], keep_ids=[], save_mask=False):
        """
            将组织图像转为HSV颜色空间 -> 中值滤波 -> 二值化阈值分割
        """
        ...
        zero = np.zeros(img_hsv.shape[:2], dtype='uint8')
        # img_hsv = cv2.merge([zero, img_med, zero])
        # img_hsv = Image.fromarray(img)
        # img_hsv.save('./wsi_core/test.jpg')
        ...
        # img_otsu = cv2.imread("/data_sda/zql/unet_42-master/test/21--56309A03_mask.png",cv2.IMREAD_GRAYSCALE)

        # img_hsv = cv2.merge([zero, img_otsu, zero])
        # img_hsv = Image.fromarray(img_hsv)
        # img_hsv.save('./wsi_core/Threshold.jpg')
        ...
        # img_hsv = cv2.merge([zero, img_otsu, zero])
        # img_hsv = Image.fromarray(img_hsv)
        # img_hsv.save('./wsi_core/close.jpg')
        ...
        save_path = False
        if save_path:
            contour = [np.array(cont // scale, dtype='int32') for cont in self.contours_tissue]
            img = np.zeros((img_otsu.shape[0], img_otsu.shape[1]), np.uint8)
            cv2.fillPoly(img, contour, 255)
            img = cv2.resize(img, (512, 512), interpolation=cv2.INTER_NEAREST)
            self.mask = Image.fromarray(img)

    def visWSI(self, vis_level=-1, color=(0, 255, 0), hole_color=(0, 0, 255), annot_color=(255, 0, 0),
        ...
        print(top_left, vis_level, region_size)
    ...
    def _assertLevelDownsamples(self):
        ...
        for downsample, dim in zip(self.wsi.level_downsample, self.wsi.level_dimensions):
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                if not blank_canvas:
                    ...
                    canvas = self.wsi.read_region(pt, vis_level, blend_block_size)
                else:
    ...
    def unet_segment(self, close=2, filter_params={'a_t': 10, 'a_h': 1, 'max_n_holes': 2}, ref_patch_size=512,
                     exclude_ids=[], keep_ids=[], mask_path=None):
        ...
        img_otsu = cv2.imread("/data_sda/zql/CLAM/unet/" + mask_path.replace('.sdpc', '.png'), cv2.IMREAD_GRAYSCALE)
        kernel = np.ones((2, 2), np.uint8)
        img_otsu = cv2.morphologyEx(img_otsu, cv2.MORPH_CLOSE, kernel)

        scale = self.level_downsamples[4]
        scaled_ref_patch_area = int(ref_patch_size ** 2 / (scale[0] * scale[1]))
        filter_params = filter_params.copy()
        filter_params['a_t'] = filter_params['a_t'] * scaled_ref_patch_area
        filter_params['a_h'] = filter_params['a_h'] * scaled_ref_patch_area
